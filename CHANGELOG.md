...
- SVG lyric hyphens now continue across adjacent measures when both notes share a system.
...
- Added an explicit cargo-deny dependency-ban policy: wildcard dependencies are rejected and
...
- Bumped the SVG metadata contract to v4 and exposed deterministic tablature string/fret
  positions, allowing browser hosts to edit tab chords without parsing SVG.
...
- Rendered ordinary-staff `technique_text` annotations in a direction-aware slot, completing the
...
- Reused those annotation-lane distances in page-margin calculation so mixed note annotations are
...
- MIDI export diagnostics now locate note annotations and grace/cue/tablature semantics that are
