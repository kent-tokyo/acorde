...
fn main() {
    ...
    m_treble.voices[0] = vec![
        quarter(Step::C, 5), quarter(Step::D, 5), quarter(Step::B, 4), quarter(Step::C, 5),
    ]; // soprano
    m_treble.voices[1] = vec![
        quarter(Step::G, 4), quarter(Step::F, 4), quarter(Step::G, 4), quarter(Step::E, 4),
    ]; // alto
    ...
    m_bass.number = 1;
    m_bass.voices[0] = vec![
        quarter(Step::E, 4), quarter(Step::D, 4), quarter(Step::D, 4), quarter(Step::C, 4),
    ]; // tenor
    m_bass.voices[1] = vec![
        quarter(Step::C, 3), quarter(Step::G, 2), quarter(Step::G, 2), quarter(Step::C, 3),
    ]; // bass
    ...
    let options = SvgRenderOptions { width: 500.0, staff_size: 24.0, measures_per_system: 4, interactive: true };
