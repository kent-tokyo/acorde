...
Platform-agnostic music score library for Rust and WebAssembly (v1.1.4).
...
```toml
acorde = { version = "1.1.4", features = ["abc", "mscz", "mei"] }
```
...
The WASM API also provides `parse_mei_render_svg` and `parse_abc_render_svg` as bounded one-call
paths from the documented MEI/ABC subsets to canonical SVG; use the corresponding `*_report`
functions when import diagnostics are needed.
